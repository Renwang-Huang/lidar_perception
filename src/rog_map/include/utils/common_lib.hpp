...
// Cleaned up duplicate macros and added std:: prefix for safety
...
namespace rog_map {
    ...
    // Replaced error-prone SIGN macro with a safe inline template
    template <typename T>
    ...
    inline Eigen::Matrix<Scalar_t, 3, 1> quaternion_to_ypr(const Eigen::Quaternion<Scalar_t> &q_) {
    ...
    template<typename Scalar_t>
    inline Scalar_t get_yaw_from_quaternion(const Eigen::Quaternion<Scalar_t> &q) {
        return quaternion_to_ypr(q)(0);
    }


    inline double computePathLength(const vec_E<Vec3f> &path) {
        if (path.size() < 2) {
            return 0.0;
        }
        double len = 0.0;
    ...
    inline Vec3f lineBoxIntersectPoint(const Vec3f &pt, const Vec3f &pos,
    ...
    inline bool GetIntersection(float fDst1, float fDst2, Vec3f P1, Vec3f P2, Vec3f &Hit) {
    ...
    inline bool InBox(Vec3f Hit, Vec3f B1, Vec3f B2, const int Axis) {
    ...
    inline bool lineIntersectBox(Vec3f L1, Vec3f L2, Vec3f B1, Vec3f B2, Vec3f &Hit) {
